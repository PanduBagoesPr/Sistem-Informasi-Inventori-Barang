...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0 text-dark">Outstocks</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="#">Home</a></li>
                    <li class="breadcrumb-item active">Outstocks</li>
                </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Outstocks</h3>
                            ...
                            <table class="table table-striped table-hover table-bordered" style="width: 100%;">
                                ...
                                @foreach($transactions as $row)
                                    <tr>
                                        ...
                                        <td>{{ ($row->total_out_stock) }} </td>
                                        <td>{{ "Rp. ".number_format($row->trxtion_price * $row->total_out_stock) }}</td>
                                        <td>{{ date('d-m-Y', strtotime($row->trxtion_date)) }}</td>
                                        <td class="text-center">
                                            <div class="btn-group">
                                                <a href="#" class="btn btn-info btn-sm" onclick="detailTransaction('{{ $row->product->product_name }}', '{{ $row->trxtion_sold }}', '{{ $row->trxtion_price * $row->trxtion_sold }}', '{{ $row->trxtion_date }}');"><i class="fa fa-eye"></i></a>
                                                <a href="{{URL::to('transaction/'.$row->trxtion_id.'/edit')}}" class="btn btn-success btn-sm"><i class="fa fa-edit"></i></a>
                                                <a href="#" class="btn btn-danger btn-sm" onclick="deleteConfirm('{{ url('transaction/delete/'.$row->trxtion_id) }}', '{{ $row->product->product_name }}');"><i class="fa fa-trash-alt"></i></a>    
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    {{ Form::text('total_out_stock', '', ['class' => 'form-control']) }}
                </div>
                <div class="form-group">
                    {{ Form::label('date', 'Date') }}
                    {{ Form::datetime('trxtion_date', '', ['class' => 'form-control', 'id' => 'datepicker']) }}
                </div>
...
<script>
    ...
    function detailTransaction(product, code, sold, price, date) {
    ...
    //     $("#edit_trxtion").attr('action', "{{ url('transaction/update') }}/"+product);
